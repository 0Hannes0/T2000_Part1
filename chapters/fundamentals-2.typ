...
Für ein reales Kundensystem wären zusätzliche Maßnahmen nötig: ein explizites Opt-in nach Art.~9 Abs.~2 lit.~a DSGVO, das Recht auf Löschung gespeicherter Embeddings nach Art.~17 DSGVO @dsgvo2016[Art.~17] und eine Datenschutz-Folgenabschätzung nach Art.~35 DSGVO @dsgvo2016[Art.~35] @voigt2017gdpr[S.~152--160]. Diese liegen außerhalb des Prototyp-Scopes. Darüber hinaus stuft die EU-KI-Verordnung (KI-Act) @euaiact2024 biometrische Echtzeit-Identifikationssysteme im öffentlichen Raum als Hochrisiko-KI ein, was bei einem produktiven Rollout zusätzliche Konformitätspflichten auslösen würde. Hinzu kommt die gesellschaftliche Akzeptanz: Nutzer akzeptieren biometrische Identifikation eher, wenn Zweck und Datenspeicherung transparent sind @rotter2008biometricacceptance[S.~68--70]. Ein öffentliches Deployment müsste einen transparenten Kamerahinweis und eine klare Opt-out-Möglichkeit früh ins Interface-Design aufnehmen.
...
In absoluten Zahlen ist die Ersparnis gering: Der Prototyp vermeidet Cloud-API-Kosten von rund \$0,90~(AWS~Rekognition) bzw.~\$1,35~(Azure~Face~API) pro Monat für die biometrische Identifikation. Der eigentliche Vorteil der lokalen Verarbeitung liegt damit nicht in der Kostenersparnis, sondern im Datenschutz --- die Biometrie verlässt das interne Netz nicht (vgl. Kap.~3.5) --- und in der Latenz: Weil kein Cloud-Call pro erkanntem Gesicht anfällt, ist die Identifikation mit ~80~ms direkt in den lokalen Erkennungszyklus integriert. Die dominierende Kostenposition im Betrieb sind die Sprachmodell-Aufrufe (gpt-4o Realtime für den Dialog), die in allen Szenarien gleich anfallen und hier nicht verglichen werden. Der wirtschaftliche Nutzen der sitzungsübergreifenden Personalisierung selbst liegt auf der Nutzenseite: Kürzere Gesprächseinstiege, weil der Assistent den Kontext bereits kennt, können die Gesprächseffizienz steigern --- eine Quantifizierung dieses Vorteils würde jedoch eine Nutzerstudie erfordern, die den Scope dieses Prototyps übersteigt.
