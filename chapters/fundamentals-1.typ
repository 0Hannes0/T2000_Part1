...
CosFace addiert die Marge direkt auf den Kosinuswert ($cos(theta) - m$), während ArcFace die Marge auf den Winkel selbst aufschlägt ($cos(theta + m)$) @deng2019arcface[S.~5--6].
Da der Kosinus eine nichtlineare Funktion des Winkels ist, erzeugt eine konstante Kosinusmarge ungleichmäßige Winkelabstände --- bei kleinen Winkeln, also ähnlichen Personen, fällt die erzwungene Trennung kleiner aus als bei bereits weitgehend verschiedenen Klassen.
Die additive Winkelmarge von ArcFace entspricht dagegen exakt der geodätischen Distanz auf der Einheitshypersphäre und legt damit entlang der gesamten Entscheidungsgrenze einen konstant linearen Abstand fest @deng2019arcface[S.~5--6], @wang2018cosface[S.~1--2].
Das Ergebnis ist eine gleichmäßig stärkere Klassentrennung über alle Klassen hinweg --- unabhängig davon, wie ähnlich sich die jeweiligen Identitäten im Trainingsraum bereits sind.
