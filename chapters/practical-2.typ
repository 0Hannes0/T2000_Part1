...
Im MTEB-Benchmark erzielt das Modell einen Durchschnittsscore von 56,53 über 56 englische Evaluationsaufgaben --- darunter 79,80 für semantische Ähnlichkeit und 58,44 für Reranking --- und positioniert sich damit im Pareto-optimalen Geschwindigkeit-Leistungs-Segment für 384-dimensionale Einbettungen @muennighoff2023mteb[S.~2020--2022].
...
Die Suche läuft über den HNSW-Index von Qdrant (logarithmische Suchzeit, vgl. Kap.~3.4) @malkov2020hnsw[S.~1--2].
...
Die RAG-basierte Kontext-Injektion birgt eine inhärente Fehlerquelle: Wenn der Retriever thematisch falsch passende Chunks zurückgibt --- weil die Anfrage zufällig ähnliche Vektorkoordinaten wie nicht themenverwandte Fakten erzeugt --- werden diese Fehlinformationen dem LLM als vermeintlich relevanter Kontext präsentiert, ohne dass das System dies erkennen kann. Dieses Risiko der stillen Fehlinjektion ist ein bekanntes Problem in RAG-Systemen, bei dem Retrieval-Fehler die Ausgabequalität aktiv verschlechtern können, ohne sichtbar als Fehler aufzutreten @karpukhin2020dpr[S.~2]. Im vorliegenden System wird diesem Risiko durch zwei Designentscheidungen begegnet: Erstens durch die atomare Granularität der `facts_sentences` (ein Fakt pro Chunk, vgl. Kap.~6.1), die verhindert, dass ein einzelnes Embedding mehrere semantisch heterogene Aussagen repräsentiert und damit Fehlzuordnungen begünstigt; zweitens durch die personenspezifische Filterung im `find_relevant_chunks(person_id, ...)`-Aufruf, die den Suchraum auf Chunks dieser Person beschränkt und Kontaminationen aus anderen Nutzerprofilen ausschließt. Das verbleibende Risiko --- thematisch ähnliche, aber inhaltlich unpassende Fakten derselben Person --- ist nicht vollständig eliminiert, sondern wird durch `k=3` und den semantischen Ähnlichkeitsschwellenwert des HNSW-Index begrenzt.
...
Die History-Isolation verhindert Cross-Contamination zwischen Nutzerprofilen: Gruppen-Chunks werden ohne Personenzuordnung gespeichert, individuelle Profilaktualisierungen finden nicht statt, und die datenschutzrechtliche Zuordnung biometrischer Fakten zu Einzelpersonen bleibt auf eindeutig identifizierbare Einzel-Sessions beschränkt @krivokucahahn2023biometricprotection[S.~639--641] (vgl. Kap.~3.5 für die datenschutzrechtliche Rahmenbewertung).
