...
In jedem Detektionszyklus ordnet der Tracker die neu eingehenden Bounding-Boxen den bekannten `_TrackedPerson`-Einträgen zu. Auf der Architekturebene relevant ist nur das _Prinzip_: Eine schnelle, positionsbasierte Stufe prüft zuerst, ob eine neue Detektion geometrisch zu einer bereits bekannten Person passt; nur wenn das nicht eindeutig ist, wird das vergleichsweise teure ArcFace-Embedding berechnet und gegen die bekannten Profile abgeglichen. Die konkreten Schwellwerte (`POSITION_MATCH_RADIUS`, `SIMILARITY_THRESHOLD`), die literaturseitige Einordnung als Tracking-by-Detection und der vollständige Algorithmus sind in Kap.~5.2 ausgeführt.
